...
const BulletIcon = styled(Box)(({ theme, active }) => ({
  width: 4,
  height: 4,
  marginLeft: "10px",
  marginRight: "8px",
...
const ChevronRightStyled = styled(ChevronRight)(({ collapsed, compact }) => ({
  fontSize: 18,
...
const BadgeValue = styled(Box)(({ sidebarCompact, theme }) => ({
  fontSize: "12px",
  fontWeight: 500,
  color: "white",
  padding: "1px 6px",
...
const SidebarAccordion = (props) => {
  ...
  useEffect(() => {
    ...
    for (let child of item.children) {
      if (child.path === window.location.href) {
        setCollapsed(true);
        setHasActive(true);
      }
    }
